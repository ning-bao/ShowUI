...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, **kwargs):
        super().__init__(config)
        if 'layer_skip_ratio' in kwargs:
            config.layer_skip_ratio = kwargs['layer_skip_ratio']
        if 'layer_skip_type' in kwargs:
            config.layer_skip_type = kwargs['layer_skip_type']
        if 'layer_skip_rand' in kwargs:
            config.layer_skip_rand = kwargs['layer_skip_rand']
            config.vision_config.layer_skip_rand = kwargs['layer_skip_rand']
        if 'merge_style' in kwargs:
            config.merge_style = kwargs['merge_style']
            config.vision_config.merge_style = kwargs['merge_style']

        if 'vis_layer_skip_ratio' in kwargs:
            config.vision_config.vis_layer_skip_ratio = kwargs['vis_layer_skip_ratio']
        if 'vis_layer_skip_type' in kwargs:
            config.vision_config.vis_layer_skip_type = kwargs['vis_layer_skip_type']
        if 'vis_layer_skip_keep' in kwargs:
            config.vision_config.vis_layer_skip_keep = kwargs['vis_layer_skip_keep']
