...
class GPTAnnotator:
    ...
    def __init__(self, api_key=None, model=None, max_tokens=None, service_tier=None, timeout_seconds=None, use_code_interpreter=None):
        ...
        # Maximum number of instructions generated by the LLM per image (default 5)
        try:
    ...
    def _preload_omniparser(self):
        ...
        try:
            ...
            # Allow configuring thresholds via env
            min_conf = 0.3
            try:
                min_conf_env = os.getenv('OMNIPARSER_MIN_CONF', '').strip()
                if min_conf_env:
  ...
  GOAL:
    From one attached screenshot, return precise, actionable UI groundings for 1–$max_elems UNIQUE elements.
  ...
  HARD RULES (PIXEL SPACE):
    ...
    4) Output an adaptive number of elements N in [1,$max_elems] based on visual richness:
         - Sparse UI: 1–2
         - Typical desktop/app: 3–4
         - Dense panels/tables/chat lists: up to $max_elems
...
<USER>
  ...
  <return>JSON only. No markdown, no commentary.</return>
        ...
        return template.substitute(width=width, height=height, hints=hints_text, crop_instruction=crop_instruction, max_elems=str(self.max_instructions))
    ...
    def _compute_preprocess_hints(self, image_path: str, max_elements: int = 5) -> List[dict]:
        ...
        if getattr(self, 'preprocess_backend', 'omni') in ('omni','omniparser'):
            ...
            if not self.omni_url or self.omni_url.lower() in ('local', 'localhost'):
                try:
                    ...
                    # Fallback to CV if no elements detected
                    if len(normalized) == 0:
                        print("[Annotator] OmniParser found 0 elements. Falling back to CV backend.")
                    else:
                        return normalized
                except ImportError:
            ...
            # Try HTTP service if URL is set
                ...
                try:
                    ...
                    print(f"[Annotator] OmniParser HTTP found {len(elements)} elements")
                    # Fallback to CV if no elements detected
                    if len(elements) == 0:
                        print("[Annotator] OmniParser HTTP found 0 elements. Falling back to CV backend.")
                    else:
                        return elements
                except Exception as e:
        ...
        print(f"[Annotator] CV backend found {len(elements)} elements")
        if len(elements) == 0:
            print("[Annotator] No regions detected by any method. Image left unannotated.")
        return elements
